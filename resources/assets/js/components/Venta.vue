                ...
                <template v-else-if="listado===0">
                    ...
                    <div class="card-body">
                        ...
                        <div class="form-group row border">
                            ...
                            <div class="col-md-2">
                                <div class="form-group">
                                    <label>Precio<span style="color: red;" v-show="precio===0">(*SELECCIONE)</span></label>
                                    ...
                                    <tbody v-if="arrayDetalle.length">
                                        <tr v-for="(detalle, index) in arrayDetalle" :key="detalle.id">
                                            ...
                                            <td>
                                                ...
                                                <input @keyup="validarVenta()" v-model="detalle.cantidad" type="number" class="form-control">
                                            </td>
                                            <td>
                                                <span style="color: red;" v-show="detalle.descuento>(detalle.precio*detalle.cantidad)">Descuento superior</span>
                                                <input @keyup="validarVenta()" v-model="detalle.descuento" type="number" class="form-control">
                                            </td>
                                            <td>
        ...
        methods : {
          // Metodo que retornara el listado de ventas
            ...
            selectCliente(search, loading) {
                ...
                axios.get(url).then(function (response) { // tod0 lo que nos devuelve el controlador
            ...
            getDatosCliente(val1) {   // Almacena toda la opcion del cliente seleccionado
                let me = this;
                me.loading = true;
                me.idcliente = val1.id; // Id sera igual al id recibido como parametro
            },
            buscarArticulo() {
                let me = this;
                var url = '/articulo/buscarArticuloVenta?filtro='+me.codigo; // Filtramos el art que coincide con el cod barra

                axios.get(url).then(function (response) {
                    var respuesta = response.data;
                    me.arrayArticulo = respuesta.articulos;

                    if(me.arrayArticulo.length>0) { // Se encontro un registro
            ...
            encuentra(id) {
                var sw = 0;
                for (var i=0;i<this.arrayDetalle.length;i++) {  // Recorre todos los indices del detalle
                    if (this.arrayDetalle[i].idarticulo===id) { // true, si ya se encuentra agregado y actualiza los detalles
                        this.arrayDetalle[i].cantidad=parseFloat(this.arrayDetalle[i].cantidad)+parseFloat(this.cantidad);
                        this.arrayDetalle[i].precio=this.precio;
                        sw = true;
                    }
                }
                return sw;
            ...
            eliminarDetalle(index) {
                let me = this;
                me.arrayDetalle.splice(index, 1);   // Eliminamos el objeto que tenga el indice index
            },
            agregarDetalle() {
                let me = this;
            ...
            validarVenta() {
                ...
                // Validamos que la cantidad de venta no supere el stock
                me.arrayDetalle.map(function (x) {
                    if (x.cantidad>x.stock) {
                        ...
                        art = x.articulo + " con descuento superior al precio de venta";
                        me.errorMostrarMsjVenta.push(art);
                    }
                });
                ...
                if(!me.num_comprobante) me.errorMostrarMsjVenta.push("Ingrese el numero del comprobante");
                if(!me.impuesto) me.errorMostrarMsjVenta.push("Ingrese el impuesto del comprobante");
                ...
                swalWithBootstrapButtons({
                    ...
                    if (result.value) {
                        ...
                        axios.put('/venta/desactivar', {
                            'id': id
                        }).then(function (response) { // Si tod0 va bien
                            me.listarVenta(1, '', 'num_comprobante');
                            swalWithBootstrapButtons(
                                'Anulado!',
